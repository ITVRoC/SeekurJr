...
#include "pan_tilt_zoom/pan_tilt_goal.h" // Customized msg for pan-tilt goal data
...
class RosAriaNode
{
  ...
  public:
    ...
    void ptzGoal_cb(const pan_tilt_zoom::pan_tilt_goal::ConstPtr &);
    void ptz_zoomIn(void);
    void ptz_zoomOut(void);
  ...
  protected:
    ...
    ArFunctorC<RosAriaNode> myZoomInCB;
    ArFunctorC<RosAriaNode> myZoomOutCB;
...
RosAriaNode::RosAriaNode(ros::NodeHandle nh) : 
  ...
  myCam(NULL),
  myZoomInCB(this, &RosAriaNode::ptz_zoomIn),
  myZoomOutCB(this, &RosAriaNode::ptz_zoomOut),
...
int RosAriaNode::Setup()
{
  ...
  // PTZ Zoom keyHandlers
  keyHandler->addKeyHandler('z', &myZoomInCB);
  keyHandler->addKeyHandler('Z', &myZoomInCB);
  keyHandler->addKeyHandler('x', &myZoomOutCB);
  keyHandler->addKeyHandler('X', &myZoomOutCB);
  robot->attachKeyHandler(keyHandler, true);
 ...
 // callback will  be called by ArRobot background processing thread for every SIP data packet received from robot
  ...
  cmdvel_sub = n.subscribe( "cmd_vel", 1, (boost::function <void(const geometry_msgs::TwistConstPtr&)>)
   ...
   ptzGoal_sub = n.subscribe("ptz_goal", 1 , (boost::function <void(const pan_tilt_zoom::pan_tilt_goal::ConstPtr&)>)
   boost::bind(&RosAriaNode::ptzGoal_cb, this, _1));
...
void RosAriaNode::ptzGoal_cb(const pan_tilt_zoom::pan_tilt_goal::ConstPtr &pan_tilt_goal_msg)
{
	robot->lock();
	myCam->panTilt(pan_tilt_goal_msg->pan, pan_tilt_goal_msg->tilt);
	robot->unlock();
}

void RosAriaNode::ptz_zoomIn(void)
{
  if (myCam->canZoom())
  {
...
void RosAriaNode::ptz_zoomOut(void)
{
  if (myCam->canZoom())
  {
    myCam->zoom(myCam->getZoom() - 
