...
AREXPORT ArCameraMode::ArCameraMode(ArRobot *robot, const char *name, 
				      char key, char key2, ros::NodeHandle *node): 
  ...
  myToggleAutoFocusCB(this, &ArCameraMode::toggleAutoFocus)
{
  ...
  ros::Subscriber ptz_goal = node->subscribe<pan_tilt_zoom::pan_tilt_goal>("/RosAria/ptz_goal", 1000, &ArCameraMode::ptzGoalCallback,this);
}
...
void ArCameraMode::ptzGoalCallback(const pan_tilt_zoom::pan_tilt_goal::ConstPtr& ptz_goal)
{
ArLog::log(ArLog::Normal, "CALLBACK BEING CALLED");
	setGoalPan(ptz_goal->pan);
	setGoalTilt(ptz_goal->tilt);
	reachGoal();
}
