...
void ROSVideoCaptureModule::_image_callback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    // Adjust caps
    _capability.width = msg->width;
    _capability.height = msg->height;
    //_capability.rawType = webrtc::kVideoI420;

    if(msg->encoding == "rgb8" || msg->encoding == "bgr8"){// force rgb8 to yuv
        ...
        uint64_t mircosec = std::chrono::duration_cast<std::chrono::microseconds>
                (m_clock.now().time_since_epoch()).count();

        IncomingFrame(yuv.data, yuv.rows * yuv.step, _capability, mircosec);
        return;
    }

    if(msg->encoding == "16UC1"){ // force 16UC1 to yuv
        ...
        cv::Mat mat16uc3_rgb(msg->width, msg->height, CV_16UC3);
        cv::cvtColor(depth_img, mat16uc3_rgb, cv::COLOR_BayerGR2RGB);

        // Convert the 16-bit per channel RGB image to 8-bit per channel
        cv::Mat mat8uc3_rgb(msg->width, msg->height, CV_8UC3);
        //double min, max;
        //cv::minMaxLoc(mat16uc3_rgb, &min, &max);
        //ROS_INFO("Max %f min %f", max, min);
        //TODO: Normalizar para distancia maxima do realsense2
        mat16uc3_rgb.convertTo(mat8uc3_rgb, CV_8UC3, 15000.0/65536.0); //this could be perhaps done more effectively by cropping bits

        // Convert to I420
        cv::Mat yuv(msg->width, msg->height, CV_8UC4);
        cv::cvtColor(mat8uc3_rgb, yuv, CV_BGR2YUV_I420);
    ...
    if(msg->encoding == "8UC1"){ // force 8UC1 to yuv
        // Copy the data into an OpenCV Mat structure
        cv::Mat ir_img = cv_bridge::toCvShare(msg, "8UC1")->image;

        // Convert 8UC1 to 8UC3
        cv::Mat rgb(msg->width, msg->height, CV_8UC3);
        cvtColor(ir_img, rgb, CV_GRAY2RGB);

        // Convert to I420
        cv::Mat yuv(msg->width, msg->height, CV_8UC4);
        cv::cvtColor(rgb, yuv, CV_BGR2YUV_I420);
    ...
    ROS_WARN("Unknow image type %s", msg->encoding.c_str());        
}
...
rtc::scoped_refptr<webrtc::VideoCaptureModule> ROSVideoCaptureModule::Create(const int32_t id, const char* deviceUniqueIdUTF8) {
    ...
    ROS_INFO_STREAM("Created CaptureModule: " << id << " " << deviceUniqueIdUTF8 );
...
int32_t ROSVideoCaptureModule::StartCapture(const webrtc::VideoCaptureCapability& capability) {
    ...
    if (_capturing) {
        if (capability.width == _capability.width &&
            ...
            /*capability.rawType == _capability.rawType balbs*/) {
    ...
    rtc::CritScope lock (&_capture_cs);
...
void ROSVideoCaptureModule::CaptureThread::Run() {
    ...
    while(!bla) {
        {
            // lock
            rtc::CritScope lock (&_parent._capture_cs);
            if (bla)
                break;
            ...
            } while (result == ros::CallbackQueue::TryAgain && !bla);
        }

        if (!bla && result == ros::CallbackQueue::Called) {
...
//balbs
ROSVideoCaptureDeviceInfo::ROSVideoCaptureDeviceInfo(const int32_t id) : DeviceInfoImpl() {
}
...
int32_t ROSVideoCaptureDeviceInfo::CreateCapabilityMap(const char* deviceUniqueIdUTF8) {
    ...
    ROS_INFO_STREAM(
        "CreateCapabilityMap " <<deviceUniqueIdUTF8 << " i '" << i << "'";
   );
    ...
    ROSWebRtcVcmFactory(
        ...
        VideoCaptureModuleRegistryPtr module_reg) :
            ...
            _module_reg(module_reg) {
                ...
                ROS_INFO_STREAM("Device id = " << device.id);
    }
...
public:
    ...
    virtual rtc::scoped_refptr<webrtc::VideoCaptureModule> Create( const char* device) {
        rtc::scoped_refptr<webrtc::VideoCaptureModule> module(
            ROSVideoCaptureModule::Create(1, device)
        );
        ROS_INFO_STREAM("Inserindo modulo Create");
        if (_module_reg != NULL) {

            ROS_INFO_STREAM("Inserindo modulo " << device);
            ROS_INFO_STREAM("Inserindo modulo " << _device_id);

            _module_reg->add(_device_id, module.get());
        }
        ROS_INFO_STREAM("Inserindo modulo Return");
        return module;
    }
...
//balbs, tirei ID
    virtual webrtc::VideoCaptureModule::DeviceInfo* CreateDeviceInfo() {
        std::unique_ptr<webrtc::VideoCaptureModule::DeviceInfo> devices(
            ROSVideoCaptureModule::CreateDeviceInfo(1)
        );
        ROS_INFO_STREAM("Inserindo modulo");
        if (devices.get() == NULL)
            return NULL;
        ROS_INFO_STREAM("Inserindo modulo1");
        ...
        if (!dynamic_cast<ROSVideoCaptureDeviceInfo *>(devices.get())->init(_topics))
            return NULL;
        ROS_INFO_STREAM("Inserindo modulo-realease");
        ROS_INFO_STREAM("Inserindo modulo");
 ...
 */
 ...
 //Balbs Olhar a questao do ID hein
...
//balbs tirando id
    virtual rtc::scoped_refptr<webrtc::VideoCaptureModule> Create( const char* device) {
        rtc::scoped_refptr<webrtc::VideoCaptureModule> module(
            //balbs
            webrtc::VideoCaptureFactory::Create( device)
        );
        if (_module_reg != NULL) {
            _module_reg->add(_device_id, module.get());
        }
...
std::unique_ptr<cricket::VideoCapturer> ROSVideoDeviceCapturerFactory::Create(const cricket::Device& device) {
    
    ROS_INFO_STREAM(
        "ROSVideoDeviceCapturerFactory::Create"
    );
    ...
    bool capt = capturer->Init(device);
 
 

    ROS_INFO_STREAM(
        "ROSVideoDeviceCapturerFactory::Init" << capt <<  "  "  
    );
...
std::unique_ptr<cricket::VideoCapturer> WebRTCVideoDeviceCapturerFactory::Create(const cricket::Device& device) {
     std::unique_ptr<cricket::WebRtcVideoCapturer> capturer(
     ...
     ROS_INFO_STREAM(
        "ROSVideoDeviceCapturerFactory::Create " << device.name <<  "  "  
        << device.id <<  "  "  
    );
