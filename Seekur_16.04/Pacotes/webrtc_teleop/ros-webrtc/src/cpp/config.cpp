...
Config Config::get(ros::NodeHandle& nh) {
    ...
    // peer bond timeouts
    instance.pc_bond_connect_timeout = 25.0;  // seconds
    ...
    instance.pc_bond_heartbeat_timeout = 15.0;  // seconds
    ...
    if (nh.getParam("ice_servers", ice_servers_xml)) {
        for (size_t i = 0; i != ice_servers_xml.size(); i++) {
            ...
            if (_get(nh, ice_servers_xml[i], ice_server)) {
                instance.default_ice_servers.push_back(ice_server);
            }
        }
    } else {
    ...
    instance.show_peer_info_topic = false;
    if (nh.hasParam("show_peer_info_topic")) {
        if (!nh.getParam("show_peer_info_topic", instance.show_peer_info_topic)) {
            ROS_WARN("'show_peer_info_topic' param type not boolean");
        }
    }
