...
PeerConnectionPtr Host::create_peer_connection(
    ...
    ROS_INFO_STREAM("Creating Peer Conection ");
    // create it
    ...
    ROS_INFO_STREAM("Starting Peer Conection ");
...
bool Host::_open_media() {
    // audio source
    _audio_src.interface = _pc_factory->CreateAudioSource(cricket::AudioOptions());
    ...
    // video sources
    ROS_INFO_STREAM("System video capture devices");
    ...
    ROS_INFO_STREAM("Ros video capture topics");
    ...
    for (int i = 0; i < (int)webrtc_video_capture_devices.size(); i++){
        ROS_INFO("\33[93m----------------------------\33[0m");
        ROS_INFO("\33[93mDevice: %d\33[0m", i);
        ROS_INFO("\33[93m----------------------------\33[0m");
        ROS_INFO("\33[93mName: %s\33[0m", webrtc_video_capture_devices[i].name.c_str());
        ROS_INFO("\33[93mFile ID: %s\33[0m", webrtc_video_capture_devices[i].file_id.c_str());
        ROS_INFO("\33[93mUnique ID: %s\33[0m", webrtc_video_capture_devices[i].unique_id.c_str());
        ROS_INFO("\33[93mProduct ID: %s\33[0m", webrtc_video_capture_devices[i].product_id.c_str());
    }
    ROS_INFO("\33[93m----------------------------\33[0m");
    ...
    for (size_t i = 0; i != _video_srcs.size(); i++) {
        ...
        switch (video_src.type) {
            ...
            case VideoSource::IdType: {
                ROS_INFO("\33[93mVideo Source Name: %s\33[0m", video_src.name.c_str());
                char* resolved_path = realpath(video_src.name.c_str(), NULL);; 
                video_src.name = std::string(resolved_path);
                ROS_INFO("\33[93mConverted to Name: %s\33[0m", video_src.name.c_str());
                auto j = std::find_if(
                    webrtc_video_capture_devices.begin(),
                    webrtc_video_capture_devices.end(),
        ...
        ROS_INFO("\33[93mCreating %s\33[0m", video_src.name.c_str());
        
        std::unique_ptr<cricket::VideoCapturer> video_capturer(
            video_capturer_factory->Create(device)
        );
        ROS_INFO("\33[93mCreated %s\33[0m" , video_src.name.c_str());
        if (video_capturer.get() == NULL) {
            ROS_ERROR("failed to create video capture device for '%s'", video_src.name.c_str());
            return false;
        }

        //ROS_INFO_STREAM("BefGetId");
        //ROS_INFO_STREAM("AftGetId");

        /*Balbs video_src.capture_module = _video_capture_modules->find(
        ...
        MediaConstraints mc;
        auto con = video_src.constraints.mandatory();

        for (auto &vla : con){
            mc.mandatory().push_back(MediaConstraints::Constraint(vla.key, vla.value));
            ROS_INFO_STREAM("Constraint " << vla.key << " " << vla.value);
        }
        // source
        ROS_INFO_STREAM("Creating Video Source " << video_src.name);
        video_src.interface = _pc_factory->CreateVideoSource(
            std::move(video_capturer), &mc
        ); 
        ...
        ROS_INFO_STREAM("Created Video Source " << video_src.name);
...
bool Host::Service::create_peer_connection(ros::ServiceEvent<ros_webrtc::CreatePeerConnection::Request, ros_webrtc::CreatePeerConnection::Response>& event) {
    ...
    PeerConnectionPtr pc(_instance.create_peer_connection(
     ...
     ROS_INFO_STREAM("Peer Connection Created");
    return pc != NULL;
}
...
bool Host::Service::send_data(ros::ServiceEvent<ros_webrtc::SendData::Request, ros_webrtc::SendData::Response>& event) {
    ...
    PeerConnectionKey key = {req.session_id, req.peer_id};
    PeerConnectionPtr pc = _instance._find_peer_connection(key);
    ROS_INFO_STREAM("Trying to send data...");
    if (pc == NULL) {
        ROS_INFO_STREAM(
            "pc (" << key.session_id << "', '" << key.peer_id << "') " <<
            "not found"
