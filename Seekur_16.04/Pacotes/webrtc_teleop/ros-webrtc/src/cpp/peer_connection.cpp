...
bool PeerConnection::create_offer() {
    _is_offerer = true;
    _pc->CreateOffer(_csdo, webrtc::PeerConnectionInterface::RTCOfferAnswerOptions());
    return true;
}

void PeerConnection::create_answer() {
    _is_offerer = false;
     _pc->CreateAnswer(_csdo, webrtc::PeerConnectionInterface::RTCOfferAnswerOptions());
}
...
void PeerConnection::status_thread(PeerConnection *a){

    
    try { /* */ 

        ROS_DEBUG_STREAM("INICIANDO THREAD DE STATUS (peer, session) ->" + a->peer_id() + "," + a->session_id());

        ros::Publisher infoPub = a->getNode().advertise<ros_webrtc::Info>("/ros_webrtc/info", 1);
        long int counter = 0;
        ...
        while(ros::ok()){
            rtc::scoped_refptr<const webrtc::RTCStatsReport> peerStats = a->GetStats();
            
            std::vector<const webrtc::RTCIceCandidatePairStats* > rtcStatsVec = peerStats->GetStatsOfType<webrtc::RTCIceCandidatePairStats>();

            if (i%100==0)
                ROS_INFO_STREAM("\33[93m"<<peerStats->ToJson()<<"\33[0m");

            i++;
            for (auto &stat :rtcStatsVec ){

                for (auto &member: stat->Members()){
                    std::string nome =   std::string(member->name());

                    if ( member->is_defined() && nome.compare("availableOutgoingBitrate")==0)
                    {
                        ...
                        msg.available_outgoing_bitrate =  *stat->available_outgoing_bitrate;
                        msg.available_outgoing_bitrate = std::min(5.0, log((msg.available_outgoing_bitrate + 1000)/1000)/log(5.5));
                        //msg.connection_rating = std::min(5.0, log((msg.available_outgoing_bitrate + 1000)/1000)/log(5.5));
                        msg.status = "connected";
                        msg.current_round_trip_time = *stat->current_round_trip_time;
                        msg.bytes_sent = *stat->bytes_sent;
                        msg.bytes_received = *stat->bytes_received;
                        msg.counter = std::chrono::duration_cast<std::chrono::seconds>(now- start_time ).count();
    ...
    //    ros::spinOnce();
        ...
        ROS_WARN( "Thread de info falhou");
    }
...
bool PeerConnection::_open_peer_connection(
    ...
    //rtc_conf.set_suspend_below_min_bitrate(true);

   // rtc_conf.disable_ipv6 = true;


    ROS_INFO_STREAM("Creating peer Connection");
    _pc = pc_factory->CreatePeerConnection(
        rtc_conf, NULL, NULL, &_pco
    );
	...
	bitrateParam.min_bitrate_bps = absl::optional<int>(100);
	bitrateParam.current_bitrate_bps = absl::optional<int>(bitrate);
	bitrateParam.max_bitrate_bps = absl::optional<int>(800000000);
	_pc->SetBitrate(bitrateParam);		

    //ROS_INFO_STREAM("Peer Connection created, adding streams");
    ...
    this->workerStatus = std::unique_ptr<std::thread>(new std::thread(PeerConnection::status_thread, this));

    

    ROS_INFO_STREAM("Peer connection opened");
...
PeerConnection::GetStats() {
  rtc::scoped_refptr<MockRTCStatsCollectorCallback> callback(
      new rtc::RefCountedObject<MockRTCStatsCollectorCallback>());
  _pc->GetStats(callback);

  //ROS_INFO("Waiting for callback info");
  while (!callback->called()){
      ros::Duration(0.5).sleep();
   }

  //EXPECT_TRUE_WAIT(callback->called(), webrtc::kDefaultTimeout);
...
bool PeerConnection::_open_local_stream(
    ...
    for (size_t i = 0; i != video_srcs.size(); i++) {
        ...
        ROS_INFO_STREAM("Open local stream: " << video_label);
        ...
        ROS_INFO_STREAM("Before Get Open local stream: " << video_label);
        ...
        ROS_INFO_STREAM("Adding track:");
        local_stream->AddTrack(video_track);
        ROS_INFO_STREAM("Created Track:");
    }

    _local_stream = local_stream;

    ROS_INFO_STREAM("Local Strem Created");
    return true;
}
...
void PeerConnection::PeerConnectionObserver::OnAddStream(rtc::scoped_refptr<webrtc::MediaStreamInterface> stream) {
   //balbs ROS_INFO_STREAM("pc ('" << instance._session_id << "', '"<< instance._peer_id << "') add stream - label: " << stream->label());
