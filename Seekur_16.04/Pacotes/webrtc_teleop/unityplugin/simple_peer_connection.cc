...
bool SimplePeerConnection::InitializePeerConnection(const char** turn_urls,
                                                    ...
                                                    bool is_receiver, int qtd_tracks) {
  ...
  g_peer_count++;
  if (!CreatePeerConnection(turn_urls, no_of_urls, username, credential, qtd_tracks)) {
    DeletePeerConnection();
    return false;
  }
...
bool SimplePeerConnection::CreatePeerConnection(const char** turn_urls,
                                                const int no_of_urls,
                                                const char* username,
                                                const char* credential, int qtd_tracks) {

	for (int i = 0; i < qtd_tracks; i++) {
		_video_observers.push_back(remote_video_observer);
		_video_observers.back().reset(new VideoObserver());
	}
...
void SimplePeerConnection::RegisterOnRemoteI420FrameReady(
    I420FRAMEREADY_CALLBACK callback) {
		...
		if (!_video_observers.empty())
			for (auto i = _video_observers.begin(); i != _video_observers.end(); i++) {
				
				(*i)->SetVideoCallback(callback);
			}
...
rtc::scoped_refptr<webrtc::MediaStreamInterface> streamGlobal;
void SimplePeerConnection::OnAddStream(
 ...
 //add multi track video stream
  int j = 0;
  for (auto i = _video_observers.begin(); i != _video_observers.end(); i++) {
	  if (j < stream->GetVideoTracks().size()) {

		  (*i)->id = streamGlobal->GetVideoTracks()[j]->id();
		  remote_stream_->GetVideoTracks()[j]->AddOrUpdateSink(
			  (*i).get(), rtc::VideoSinkWants());
		  j += 1;
	  }
	  else {
		  (*i) = NULL;
...
bool SimplePeerConnection::CreateDataChannel() {
  ...
  init.protocol = "application/vnd.rosbridge.v1+json; chunksize=0";
  init.id = 1;
  init.ordered = false;
  init.reliable = false;

  data_channel_ = peer_connection_->CreateDataChannel("rosbridge", &init);
