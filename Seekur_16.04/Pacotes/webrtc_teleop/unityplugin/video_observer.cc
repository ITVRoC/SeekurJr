...
VideoObserver::VideoObserver( std::string  id, webrtc::VideoTrackInterface* video_track) :_video_track(video_track){
	//this->id = "888";
	_video_track->AddOrUpdateSink(this, rtc::VideoSinkWants());

}
webrtc::VideoTrackInterface* VideoObserver::video_track() {
...
void VideoObserver::OnFrame(const webrtc::VideoFrame& frame) {
  ...
  if (buffer->type() != webrtc::VideoFrameBuffer::Type::kI420A) {
    ...
    OnI420FrameReady(i420_buffer->DataY(), i420_buffer->DataU(),
                     ...
                     frame.width(), frame.height(), id.c_str());
